...
/**
 * Sample data and interfaces for that data
 */
...
const articleInfo: Array<ArticleInfo> = [
...
function generateTitle(teamId: string, index: number): string {
	return (
		teamId[0].toUpperCase() +
		teamId.slice(1) +
		' ' +
		articleInfo[index].title
	);
}
...
export function generateArticle(teamId: string, articleId: string): Article {
	const index = articleInfo
		.map((info: ArticleInfo, index: number): string =>
			slug(generateTitle(teamId, index))
		)
		.indexOf(articleId);
...
export interface TeamsArticle {
	date: Date;
	title: string;
	id: string;
}

export function generateTeamsArticles(teamId: string): Array<TeamsArticle> {
	return articleInfo.map(
		({ title, date }: ArticleInfo, index: number): TeamsArticle => {
